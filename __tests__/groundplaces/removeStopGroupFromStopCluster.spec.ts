...
describe('#removeStopGroupFromStopCluster', () => {
  ...
  it('should throw an error if the StopGroup to remove is lonely after the operation', () => {
    ...
    expect(thrownError).toEqual(
      new Error(
        'Impossible to remove the StopGroup with the Gpuid "g|FRnanvanna@u0skgb" from the StopCluster with the Gpuid "c|FRnaarto__@u0skg". Make sure that the StopGroup will not be alone after this operation.',
      ),
    );
    expect(groundPlacesService.getGroundPlaces()).toStrictEqual([
      {
